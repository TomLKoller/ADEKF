...
public:
    ...
    template <typename OtherScalar, int N> inline
    static ceres::Jet<OtherScalar, N> norm(const ceres::Jet<OtherScalar, N>& a, const ceres::Jet<OtherScalar, N>& b) {
	...
	auto operator+(const Eigen::MatrixBase<Derived> &delta) const {
        DirectionVector<ADEKF_PLUSRESULT(Scalar,OtherScalar)> temp(getRx<Scalar>(*this)*getExp<OtherScalar>(delta));
	    assert_finite(temp);
        return temp;
	}

	template<typename OtherScalar>
	Eigen::Matrix<ADEKF_MINUSRESULT(Scalar,OtherScalar),DOF,1> operator-(const DirectionVector<OtherScalar> &other) const {
