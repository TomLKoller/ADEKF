...
#include "../../include/UKF/UKF.hpp"
int main(){
    //Initialize adekf with 6 states
    adekf::UKF ukf(Eigen::Vector3d::Zero(),Eigen::Matrix3d::Identity());
    ...
    ukf.predict(dynamic_model,__IN_processNoise,__IN_acceleration,__IN_time_diff);

    //Correct estimation with a measurement update
    ukf.update(measurement_model,__IN_measurement_noise,__IN_velocity);


    //print out ekf state
    std::cout << ukf.mu << std::endl;
