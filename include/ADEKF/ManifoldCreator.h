...
#define ADEKF_CONSTRUCTOR_ARGS_NO_DEFAULT(name)  decltype(name) && arg_##name
#define ADEKF_CONSTRUCTOR_ARGS(name) ADEKF_CONSTRUCTOR_ARGS_NO_DEFAULT(name)=decltype(name)()
...
#define ADEKF_CONSTRUCT_MANIFOLD_INTERNAL(name, members) \
ADEKF_CONSTRUCTOR(name, members) \
using ScalarType = T; \
static constexpr unsigned DOF = 0 ADEKF_TRANSFORM(ADEKF_ADD_DOF,members); \
ADEKF_BOXPLUS(name, members) \
ADEKF_BOXMINUS(name, members) \
ADEKF_OUTSTREAM(name, members) \
ADEKF_INSTREAM(name, members) \
EIGEN_MAKE_ALIGNED_OPERATOR_NEW\
};\
	ADEKF_DEDUCTION_GUIDE(name,members)
...
#define ADEKF_MANIFOLD_ONLY_MANIFOLDS(name, manifolds) template<typename T> struct name :public adekf::Manifold\
{\
...
#define ADEKF_MANIFOLD_BOTH(name, manifolds, vectors) template<typename T> struct name :public adekf::Manifold\
{\
	ADEKF_UNZIP_ATTRIBUTES(CREATE_ATTRIBUTE,manifolds)             \
...
#define ADEKF_STATE(name, ...) template <typename T> struct name :public adekf::Manifold{\
...
#define ADEKF_PARSE_MANIFOLD(size, name,manifolds, ...) EXPAND_(BOOST_PP_IF(size,0,1),name,manifolds,__VA_ARGS__)
